...
class TradeApi(private val rpcOps: CordaRPCOps) {
    ...
    @PUT
    @Path("create-trade")
    fun createTrade(@QueryParam("sellValue") sellValue: Int,@QueryParam("sellCurrency") sellCurrency: String, @QueryParam("longContract") longContract: Int, @QueryParam("counterParty") counterParty: CordaX500Name?,@QueryParam("tradeStatus") tradeStatus: String): Response {
        ...
        if (longContract < 0 ) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'Contract period' must be 0 or higher.\n").build()
        }
        ...
        return try {
            val signedTx = rpcOps.startTrackedFlow(::Initiator, sellValue, sellCurrency, longContract, tradeStatus, counterParty).returnValue.getOrThrow()
    ...
    @PUT
    @Path("counter-trade")
    fun counterTrade(@QueryParam("tradeId") tradeId: String,@QueryParam("sellValue") sellValue: Int,@QueryParam("sellCurrency") sellCurrency: String, @QueryParam("longContract") longContract: Int, @QueryParam("counterParty") counterParty: CordaX500Name?,@QueryParam("tradeStatus") tradeStatus: String): Response {
        if (counterParty == null) {
            return Response.status(BAD_REQUEST).entity("Query parameter 'Counter partyName' missing or has wrong format.\n").build()
        }
        val counterParty = rpcOps.wellKnownPartyFromX500Name(counterParty) ?:
                return Response.status(BAD_REQUEST).entity("Counter Party named $counterParty cannot be found.\n").build()
        return try {
            val signedTx: SignedTransaction = rpcOps.startFlowDynamic(CounterTradeFlow.CounterInitiator::class.java, sellValue,sellCurrency,tradeStatus,longContract,tradeId,counterParty).returnValue.getOrThrow()
            Response.status(CREATED).entity("Transaction id ${signedTx.id} committed to ledger.\n").build()
